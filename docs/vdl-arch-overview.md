...
1. **Acquire** (stages 1–3) — crawl the VDL catalog, download the source DOCX/PDF
   files, and convert them to GitHub-flavored markdown with extracted images.
...
(canonical cross-reference store, stages 6–6.7).
...
flowchart TD
    ...
    subgraph ACQUIRE["Acquire (pipeline.db)"]
        ...
        S3["Stage 3 · INGEST<br/>Pandoc DOCX to MD + images"]
    end
...
| 3 | **INGEST** (`vista-docs ingest`) | Convert DOCX→GitHub-flavored markdown via Pandoc 3.x; extract embedded images; scaffold a frontmatter stub. | `raw/{app_code}/*.docx` + Pandoc | `md-img/{app_code}/{stem}.md` + `{stem}/NNN.png`; `pipeline.db ▸ manifest` (ingest_status, markdown_path) |
| 4 | **ENRICH** (`vista-docs enrich`) | Parse markdown body; extract metadata (word/page count, revision history, stub flag, security keys, menu options, keywords, package ns/version, audience…) and populate YAML frontmatter in-place. | `md-img/{app_code}/*.md` | Same files, frontmatter fully populated in-place |
