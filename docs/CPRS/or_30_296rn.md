...
patch_gap: null
section: ''
source_file: or_30_296rn.docx
status: draft
title: or 30 296rn.docx
---
...
Release Method: Monitored Release	3
...
Combat Veteran Markers	6

Mental Health	10

Defect Fixes	11

Bidirectional Health Information Exchange (BHIE)	11

CPRS Splash Screen	11

Delayed Orders/Encounter Information	12

Display Issue (PSPO 1089) Warning	12

Document Searches	12

Encounters	12

Encounter Location Switching Form	13

Graphing	13

Labs Tab	13

Medication Quick Orders	14
...
Developers changed CPRS to require a unique schedule when leaving the schedule field in the Unit Dose dialog.

Developers changes what happens when the user presses the &lt;Enter&gt; key while the focus is still on a drop-down list. Previously, pressing the &lt;Enter&gt; key selected the item and accepted the order, but now pressing the &lt;Enter&gt; key will be the same as pressing the &lt;Tab&gt; key—it will select the item and move to the next field.

CPRS will also check for a control character in the schedule field when placing an order. If CPRS finds a control character, then it displays an error message to the user and the order cannot be saved until the schedule field is corrected.
...
**CPRS Support for the New Mental Health DLL** – The Mental Health package created a patch to address an issue with temporary files. To avoid confusion with the previous dynamic link library (DLL), the name of the DLL was changed. CPRS made the changes necessary to support the changes to the new DLL, YS\_MHA\_A.DLL.
...
Resolution: Developers changed  quick orders to accept a fractional amount for an infusion rate.
...
**Convert IV Inpatient QO to Infusion QO Has a Typo** – There was a typographical error in the text for the conversion utility. The word “move” should have been “moved”.
...
Launch the Inpatient Medications order dialog.

Select the Complex tab.

In lower case (this must be lower case), type the first few characters of a schedule such as BID —choose a schedule for which the first few characters are NOT unique (i.e., BID is not enough to unique match between BID and BID AC). Or, type in an invalid schedule.

Without leaving the Schedule field, press CTRL+ENTER to activate the default button on the form (Accept Order).
