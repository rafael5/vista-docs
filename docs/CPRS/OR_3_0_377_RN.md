...
patch_gap: null
section: ''
source_file: OR_3_0_377_RN.docx
status: draft
title: Computerized Patient Record System GUI v31b
---

# Computerized Patient Record System GUI v31b
...
4.1.	Feature from Outside CPRS that Affect CPRS Functionality	2

4.1.1.	PIN Entry Required for Each Controlled Substance Order	2

4.2.	New Features and Functions Added	2

4.2.1.	Enhanced Cover Sheet	2

4.2.2.	New SMART Features (NSR 20100701)	4

4.2.2.1.	SMART Alerts	5

4.2.2.2.	Deferring Alerts	5

4.2.2.3.	New Patient-Centric Notifications View	5

4.2.2.4.	Viewing and Processing SMART Notifications	6

4.2.3.	CPRS.Women's Health Potentially Unsafe Medications, NSR 20071218	9

4.2.4.	Other Than Honorable (OTH) Functionality (NSR 20170403)	10

4.2.5.	Copy/Paste - The Ability to Identify and Monitor (NSR 20080528)	11

4.2.6.	Set Consult TIU Templates as Read-only	12

4.2.7.	Dietetics: Prevent Previous Active Diet Replacing NPO Order Upon Expire, Discontinue, or Cancel (NSR 20080323)	13

4.2.8.	Long Text Informational Alerts	14

4.2.9.	Order Check History, NSR 20140806	14

4.2.10.	Rehost/Reengineer Primary Care Management Module (PCMM) Work Effort Unique Identifying, NSR 20070415	14

4.3.	Patient Safety Issues	15

4.4.	Defect Tracking System Items	19

4.5.	Known Issues	31

5.	New Parameters	32

5.1.	CPRS Parameters	32

5.2.	Consults Parameters	32

5.3.	TIU Parameters	32

5.4.	Women’s Health Parameters	33

6.	Modified Parameters	33
...
Figure 7 The OTH button outlined in red for emphasis
...
The NPO orderable item will be added to the Auto-DC Rules when the patch is installed. With this change, the auto-DC rules will prevent the discontinuation of the NPO diet orders for the following event types:

- Specialty transfer (S)
- OR (O)
- Transfer (T)

and the NPO diet remains intact. When NPO diet orders are attempted to be manually discontinued, the system will prevent that action from being taken and display the dialog box to the user with text “NPO Diet cannot be discontinued”.

The system will block all diet orders from having an Expiration Date/Time when ordered and prevent automatic reinstatement of a previous diet. If a new diet order is desired, it must be ordered to replace the active diet order.

When ordering NPO, if a patient is currently on tube-feeding, the provider is displayed in the dialog box, “The patient currently has an active tubefeeding order &lt;display the current tube-feeding order here&gt;”.

The Cancel Future Order Prompt from the tubefeeding order dialog has been removed. A conversion routine has been created to remove the prompt from any Quick Order. Additionally, Quick Orders will no longer allow the features of Auto Accept or Verify.

Accepting a tubefeeding order will prompt one of two different messages depending on the current diet status of the patient. If there is no existing order, the user will be prompted to enter a new diet order. If an existing diet order is present, the user will be prompted to keep the current diet or enter a new diet order.
...
1. The Source Combinations screen (Tools &gt; Options &gt; List/Teams tab) now includes PCMM as a source option.

1. The Patient Selection Defaults screen (Tools &gt; Options &gt; List/Teams tab &gt; Patient Selection Defaults) now includes PCMM Team as an option for the List Source, and a dropdown list to choose a default PCCM Team.

1. Source Combinations (Tools &gt; Options &gt; List/Teams tab &gt; Source Combinations) now offers PCMM as a source option.

1. Team Information (Tools &gt; Options &gt; List/Teams tab &gt; Teams Information) has added a checkbox to include PCMM teams with descriptive text indicating that PCMM teams may not be edited via CPRS.
...
Resolution: CPRS Cover Sheet will now display the full name of the immunization instead of the short name (i.e. “Pneumococcal Polysaccharide Ppv23” instead of “Pneumovax”). Reference SDM ticket R10995796FY17. **Note:** There is one remaining issue to completely correct the Log Name issue and it will be released in OR*3*498.
...
INC000000067447 HIN-0102-41933  Review/Sign Changes Asks Nature Of Order For Health Factor
...
INC000000069287  PAL-0600-62575  Order Set Doesn't Work right
...
INC000000069337  FAR-1100-41820  PPD Reminder Dialog does not send over a 0 reading
...
INC000000069685  HIN-0302-41180  MISSING ERROR WHEN LINK REMINDER DIALOG TO CONSULT REQUEST
...
INC000000069758  4 UNY-0502-12261  Dose/Draw times inappropriate/incorrect
...
INC000000070504  PUG-0104-51406  Rem dialogs not respecting indent of W-P temp field
...
INC000000133674 Problem with Edit/Resubmit in CPRS.
...
INC000000205811 Name not being capitalized in Primary Care Team/Provider dialog box
...
INC000000210982 (HITPS-0391, 0751, 1172, 1843) Auto-DC rules changed to prevent discontinuation of NPO diet orders for the Specialty transfer (S), OR (O), Transfer (T) event types
...
INC000000214447 Quantity/Days Supply issues when entering an Outpatient Order
...
INC000000215908 Menus in order set automatically moving to next screen
...
INC000000221092 “stacked” alerts
...
INC000000240130  VAMHCS - TIU*1*222 printing issue
...
INC000000245545 Can copy an order with an inactive drug
...
INC000000301412 “No Report Generated” warning not clearing between tabs
...
INC000000305468 Users getting kicked out of CPRS with access violations
...
INC000000319910 Marking SHAD for encounters
...
INC000000320527 Delayed Orders instruction confusion
...
INC000000328153 Need to add logging capability into CPRS.
...
INC000000394902 Diagnosis form problem.
...
INC000000433206 AP reports from another VA showing up in patient CPRS record
...
INC000000434226 When Surgery tab is selected for a certain patient, the user gets an invalid index error
...
INC000000443011 Encounter Pop-Up Box Not Appearing with Flu Reminder as Secondary Diagnosis
...
INC000000450139 Pt name on reminder definition in CPRS
...
I13413770FY17  Non-VA Meds &amp; Additional Diet orders NOT releasing v28
...
INC000000482781 V28 Release from Hold Action
...
INC000000536815 PCE software/issue
...
INC000000583563  Inactive Dietary Tube Feed Quick Order still functions
...
INC000000603534 A note title association got somehow LINKED to the ROOT folder “Document Titles”.
...
INC000000444608 Discharge Summary - Attending Physician
...
INC000000594883: scrambled signature block
...
INC000000853662  OERR - Orders Tab
...
INC000001051196 (HITPS-1922) Problem Forwarding Consult - Selection Jumps to Different Service
...
Restrict One-Step Clinic Administration Menu
...
R6418568FY16: CPRS Encounter Issue – Problem selecting appropriate diagnosis
...
I7439454FY16: Lab Order Discontinue Issue
...
I10799607FY16, I9235462FY16, I8633754FY16, I14165711FY17 CPRS crashes with certain order sets
...
R9635898FY16 (HITPS-2251, 2260, 2316) Missing data objects - CPRS update
...
R10995796FY17 (HITPS-2298) CPRS Coversheet immunization display issue due to Immunization Short Name – Patient Safety Issue reported
...
I16877892FY18 Problem with messages after OR*3*461 released
...
I17427419FY18: BCMA Radio Buttons not properly displaying warning messages
...
I16805536FY18: Receiving access violations daily.
...
I18660001FY18 Error in ORQPTQ5
...
I18906933FY18 - CPRS GUI Access Violations when user selected a patient and, in another case, when she processed an alert
...
INC000000914776 (HITPS-1819) Patient selection screen issue
...
I6910726FY16 Display Issue in CPRS for IV Order
...
I7705227FY16 - Pharmacy Quick Orders
...
INC0419622 - RA OERR EXAM Menu does not allow user to scroll through all exams
...
INC0000002148685 - RTC Order Cancellation IEN issue
...
I19275515FY18 - VistaWeb button label issue in CPRS
...
## 8 CPRS v31B Patch Description Changes
...
Outpatient Pharmacy -		PSO\_7\_MAN\_UM.DOCX	Binary

Manager's User Manual	PSO\_7\_MAN\_UM.PDF	Binary

Outpatient Pharmacy -		PSO\_7\_PHAR\_UM.DOCX	Binary

Pharmacist's User Manual         PSO\_7\_PHAR\_UM.PDF	Binary

Outpatient Pharmacy -		PSO\_7\_SUP\_UM.DOC		Binary

Supplemental User Manual	PSO\_7\_SUP\_UM.PDF		Binary

Outpatient Pharmacy -		PSO\_7\_TECH\_UM.DOCX	Binary

Technician's User Manual	PSO\_7\_TECH\_UM.PDF	Binary
...
OR\_3\_0\_377\_RN.PDF		CPRS GUI v.31 (Patch OR*3.0*377) Release Notes

CPRSV31B\_REQUIRED.KID	CPRS v31B multi-package build

CPRS\_31\_Help.ZIP			This file is not distributed in an individual ZIP file.

The HELP folder is now contained in OR\_30\_377.ZIP.
...
exported as or\_3\_0\_377\_dibr.docx and or\_3\_0\_377\_dibr.pdf and is included

in OR\_30\_377.ZIP and will be emailed to the installation points of contact
