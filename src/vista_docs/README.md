...
| 9.5 | `normalize` | `vista-docs normalize` | Convert lossless `consolidated/` markdown into clean gold markdown: denoise (F1), unwrap dead pandoc TOC/nav-link wrapping `[[text](#_Toc)]` (F3a), strip running headers/footers (F2), infer headings (F3), assign GitHub-slug anchors + legacy-bookmark alias map (F4), demote the revision-history table to a `*.history.yaml` sidecar + frontmatter summary and de-pollute `description` (F5), generate a link TOC (F6), page-number bridge for paginated-only docs (F7), recover figure captions (F9), normalize tables (F10), rewrite legacy `#…` links + dead-anchor sweep (F8). Stamps provenance (`source_sha256`, `converter`, `normalized_at`, `normalize_version`). Idempotent + reversible (see `docs/publish-normalization-spec.md`). | `consolidated/**/*.md` (never mutated), `raw/` (for `source_sha256`) | `normalized/{app}/{doc_type}/{title}.md` + `*.history.yaml` sidecars (mirror tree) | `consolidate` |
| 10 | `publish` | `vista-docs publish` | Write the human-browsable publish tree: anchor docs at `{section}/{pkg}/`, patch docs under `patches/`, image dirs copied alongside each `.md`. **Prefers `normalized/` gold bodies** when present (per-doc fallback to `consolidated/`; images always from `consolidated/`). | `normalized/` (if present), `consolidated/`, `md-img/`, `migration/corpus-manifest.json`, `inventory/vdl_inventory_enriched.csv` | `publish/{section}/{pkg--full-name}/{label[--variant]}.md`; `publish/{section}/{pkg}/patches/{patch-id}--{label}.md`; `publish/INDEX.md` | `consolidate` + `manifest` (+ `normalize` for gold bodies) |
