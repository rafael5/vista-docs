...
def table_header_tokens(table) -> frozenset:
    ...
    return frozenset(re.split(r'[\s,/|]+', header_text))
...
def find_docx_files(manifest_path: Path, pkg_filter: str | None,
    ...
    """
    Find all fetched .docx files from a manifest.

    Supports two manifest formats:
      - Pilot format:  manifest["packages"][pkg_key]["documents"][...]
                       files expected at raw/{pkg_key}/{filename}
      - Guides format: manifest["documents"][...]  (flat list)
                       files expected at docs/{app_code}/{filename}
                       identified by presence of top-level "documents" key
                       without a "packages" key.
    """
    with open(manifest_path, encoding="utf-8") as f:
        manifest = json.load(f)

    # ── Normalise: guides flat format → pilot-style packages dict ────────────
    is_guides_format = "documents" in manifest and "packages" not in manifest
    if is_guides_format:
        # Synthesise packages structure in-memory; one package per app_code
        packages: dict = {}
        for doc in manifest["documents"]:
            if doc.get("fetch_status") not in ("ok", "ok-docx", "ok-pdf"):
                continue
            pkg_key = (doc.get("app_code") or "unknown").lower()
            if pkg_key not in packages:
                packages[pkg_key] = {"documents": []}
            # Reconstruct local_path as docs/{APP}/{filename}
            app_code = doc.get("app_code") or "unknown"
            fname    = doc.get("filename", "")
            packages[pkg_key]["documents"].append({
                "is_base":    True,   # all guides docs are treated as base
                "local_path": f"docs/{app_code}/{fname}",
                "docx_filename": fname,
            })
    ...
    for pkg_key, pkg in manifest["packages"].items():
        if pkg_filter and pkg_key.lower() != pkg_filter.lower():
            continue
        for doc in pkg["documents"]:
            if not doc.get("is_base"):
                continue
            # Try local_path first (set by fetch.py or synthesised above)
